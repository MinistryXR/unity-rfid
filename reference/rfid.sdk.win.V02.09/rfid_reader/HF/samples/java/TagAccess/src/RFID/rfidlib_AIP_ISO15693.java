...
public class rfidlib_AIP_ISO15693 {
	...
	public static void LoadLib(String sLibPath, int osType, int arType)
	{
		...
		{
			libPath = String.format("%s/libs/%s/%s/rfidlib_aip_iso15693.dll",
					sLibPath, osName, architecture);
			System.load(libPath);
			libPath = String.format("%s/libs/%s/%s/jni_rfidlib_aip_iso15693.dll",
					sLibPath, osName, architecture);
			System.load(libPath);
		}
	...
	*/
	...
	public native static int ISO15693_SetInvenReadParam(
			long hIso15693InvenParam, 
			byte readSecSta, 
			int blkAddr,
			int numOfBlks);
	
	public native static int ISO15693_ParseInventoryReadReport(long hTagReport,
			byte uid[],
			Integer numOfBlocks,
			byte blkData[],
			Integer nSize);
