...
CHexEdit::CHexEdit()
{
	...
	m_length		= 0x40;
	m_topindex		= 0;
	m_bpr			= 5;		// byte per row 
	m_lpp			= 1;
	m_aiu			= 1;		// address increase unit
	m_sa			=0 ;
	m_bShowHex		= TRUE;
	m_bShowAscii	= TRUE;
	m_bShowAddress	= TRUE;
...
void CHexEdit::OnPaint() 
{
	...
	if(m_pData)
	{
		...
		if(m_bShowAddress)
		{
			...
			for(int	 i = m_topindex; (i < row ) && (rcd.TopLeft().y < height); i+= m_aiu)
			{
				_stprintf(buf, fmt, m_sa+ i);
		...
		if(m_bShowHex)
		{
			...
			{
				int j =m_topindex * m_bpr;
				for(int	 i = m_topindex; (i < row ) && (rcd.TopLeft().y < height);i++ )
				{
					TCHAR* p = &buf[0];
					int	  n;
					for(n = 0; (n < m_bpr) && (j < m_length); n++)
					{
						TOHEX(m_pData[j], p);
						*p++ = ' ';
						j++;
...
void CHexEdit::UpdateData(LPBYTE p,int offset,int len)
{
	if(m_length < (offset+len) ) 
		len = m_length - offset ;
	memcpy(m_pData+offset,p,len) ;
...
int CHexEdit::GetData(LPBYTE p, int offset,int len)
{
	int nCopy ;
	nCopy = min(len, (m_length- offset));
	memcpy(p, m_pData+offset, nCopy);
	return nCopy;
}